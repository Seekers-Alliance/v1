...
export default function ProbabilityTree() {
    ...
    const tokenIDNames=['ID-001','ID-002','ID-003','ID-004','ID-005','ID-006']
    ...
    const data=drawingPoolNames.map((poolName, index) => {
        return {
            ...
            children: unitPoolNames.map((unitPoolName, index2) => {
                return {
                    key: `0-${index}-${index2}`,
                    title: <Title className='text-[16px]'>{unitPoolName} <span className='text-[#ADC0F8]'>10%</span></Title>,
                    children: tokenIDNames.map((tokenIDName, index3) => {
                        return {
                            key: `0-${index}-${index2}-${index3}`,
                            title: <Title className='text-[12px]'>{tokenIDName} <span className='text-[#ADC0F8]'>10%</span></Title>,
                        }
                    })
                }
            })
