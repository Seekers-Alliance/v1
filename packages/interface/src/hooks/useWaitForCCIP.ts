...
import {useContractEvent, useContractWrite, usePublicClient, useWaitForTransaction} from 'wagmi';
import useDrawingWrite from '@/hooks/useDrawingWrite';
import {useCallback, useEffect, useMemo, useRef, useState} from 'react';
import {Hash, parseAbi} from 'viem';
import {filterMarketplaceEvents, mappingMessageReceivedParams} from "@/core/events/marketplace";
import {getEvent} from "@/core/events/event";
import {useAddresses} from "@/hooks/useAddresses";
import {MessageReceivedParams} from "@/core/types";

export default function useWaitForCCIP(chainId: number,senderHash?: `0x${string}`) {
  ...
  const {
    ...
    chainId: chainId, hash: senderHash
  })
  ...
  const checkReceived=useCallback( (log:any)=>{
    return log.args.messageId===messageIdRef.current
  },[messageIdRef.current])
  const handleReceived=useCallback((log:any)=>{
