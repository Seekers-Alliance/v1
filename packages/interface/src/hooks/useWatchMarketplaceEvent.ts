...
export default function useWatchMarketplaceEvent(fn:string,filter:(event:any)=>boolean) {
    const {marketplaceReceiverAddress,marketplaceSenderAddress}=useAddresses()
    ...
    const fetchFn=useCallback(async ()=>{
        ...
        // @ts-ignore
        const sorted=filtered.sort((a,b)=>-1*(a.blockNumber-b.blockNumber))
        return sorted[0]
    },[client,address,fn])
