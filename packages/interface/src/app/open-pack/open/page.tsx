    ...
    <main className='flex min-h-screen bg-black bg-[url("/purchase.png")]'>
      <div className='fixed left-[529px] top-[486px]'>
        <div className='flex h-[140px] w-[383px] flex-col justify-between gap-4'>
          <OpenPackButton packId={0} poolAmount={poolAmount}>{`OPEN ${packAmount} PACKS*`}</OpenPackButton>
          <OpenPackButton packId={1} poolAmount={1}>{'OPEN SPECIAL PACK'}</OpenPackButton>
        </div>
      </div>
    </main>
  );
  ...
  switch (status) {
    ...
    case OpenStatus.Open:
        return (
            <>
                {contextHolder}
                <PrimaryButton loading={isLoading} onClick={handleOpenPack}>{children}</PrimaryButton>
            </>
        );
